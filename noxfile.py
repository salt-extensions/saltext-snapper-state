...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=tests/*",
            "--include=src/saltext/snapper_state/*",
        )
        ...
        session.run(
            ...
            "--omit=src/saltext/snapper_state/*",
            "--include=tests/*",
        )
        try:
            session.run(
                "coverage", "report", "--show-missing", "--include=src/saltext/snapper_state/*"
            )
            ...
            #    "--include=src/saltext/snapper_state/*,tests/*"
            # )
        finally:
